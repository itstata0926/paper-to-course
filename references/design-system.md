# Design System Reference (paper-to-course)

Complete CSS design tokens for the course. The tokens below are already baked into `references/styles.css` — this file is for reference when composing module HTML, not for regenerating CSS.

## Chinese Typography Notes (paper-to-course)

The course body is written in Chinese. `styles.css` already adds `'Noto Sans SC', 'PingFang SC', 'Microsoft YaHei'` fallbacks to all three font vars, and `_base.html` loads Noto Sans SC from Google Fonts. When writing module HTML:

- **Punctuation:** use full-width Chinese punctuation（，。：？"" etc.）in Chinese prose; keep half-width in English quotes, code, and citations.
- **CN/EN spacing:** put a half-width space between Chinese and inline English/numbers（例如 "HR 0.70 表示…"）— do it manually in the HTML text; there is no CSS that fixes this.
- **Line length:** Chinese reads denser than English — keep paragraphs to 2-3 sentences; the "max 3-4 short paragraphs per screen" rule is even more important in Chinese.
- **Display headings:** module titles can mix a short English kicker (mono, small) with a Chinese main title (display font) for visual rhythm.
- **Numbers & stats:** always half-width digits; keep units tight to the number (4.2 个月, HR 0.70, 95% CI 0.55–0.89).
...
:root {
  ...
  /* --- ACTOR COLORS (assign to main components) ---
     A warm, accessible palette sampled from screenshots.
     Each major "character" in the codebase gets a distinct color
     for chat bubbles, diagrams, and highlights. */
  --color-actor-1:        #D37C5F;       /* terracotta */
  --color-actor-2:        #6A9BCC;       /* dusty blue */
  --color-actor-3:        #8782BF;       /* muted purple */
  --color-actor-4:        #82936A;       /* olive / moss */
  --color-actor-5:        #6B9F8D;       /* sage teal */
}
```
