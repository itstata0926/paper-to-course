...
/* ================================================================
   PAPER-TO-COURSE ADDITIONS
   Toggle behaviors for discussion cards and briefing-pack Q&A.
   ================================================================ */
(function () {
  'use strict';

  /* ── DISCUSSION CARD TOGGLE ────────────────────────────────── */
  /* Usage: <button class="discussion-toggle" onclick="toggleDiscussion(this)">...</button>
            followed by a sibling <div class="discussion-angles">...</div> */
  window.toggleDiscussion = function (btn) {
    const card = btn.closest('.discussion-card');
    if (!card) return;
    const angles = card.querySelector('.discussion-angles');
    if (!angles) return;
    const isOpen = angles.classList.toggle('open');
    btn.classList.toggle('open', isOpen);
    btn.textContent = isOpen ? '收起' : '展开';
  };

  /* ── BRIEFING PACK Q&A ACCORDION ───────────────────────────── */
  /* Usage: <div class="qa-item">
              <button class="qa-question" onclick="toggleQA(this)">
                Question text <span class="qa-chevron">▾</span>
              </button>
